...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const data = await prisma.hargaSampah.findFirst({ where: { id, deletedAt: null } })
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const data = await prisma.hargaSampah.update({ where: { id }, data: parsed.data })
    return Response.json(data)
  } catch (e) {
    ...
    return Response.json({ error: "gagal mengupdate harga sampah" }, { status: 400 })
  }
}
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params
  try {
    await prisma.hargaSampah.update({ where: { id }, data: { deletedAt: new Date() } })

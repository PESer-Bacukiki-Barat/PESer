...
describe("GET /api/nasabah", () => {
  it("mengembalikan daftar", async () => {
    ...
    expect(await json(res)).toEqual([{ ...validBody, id: "n1" }])
  })
...
describe("POST /api/nasabah", () => {
  ...
  it("400 untuk body tidak valid", async () => {
    mAuth.mockResolvedValue(authOk)
    expect((await POST(body({ rt: "" }))).status).toBe(400)
  ...
  it("409 jika kodeNasabah sudah dipakai (P2002)", async () => {
    ...
    expect(await json(res)).toEqual({ error: "kodeNasabah sudah dipakai" })
  })

  it("400 jika bankSampahId tidak ditemukan (P2003)", async () => {
    mAuth.mockResolvedValue(authOk)
    m.create.mockRejectedValue(prismaError("P2003"))
    const res = await POST(body())
    expect(res.status).toBe(400)
    expect(await json(res)).toEqual({ error: "bankSampahId tidak ditemukan" })
  })
